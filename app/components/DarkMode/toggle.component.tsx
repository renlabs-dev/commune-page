  ...
  return (
    <div
      className={`${currentTheme === theme ? 'bg-gray-100 dark:bg-[#131B2A]' : ''
        } flex w-full items-center gap-2 px-4 py-2 text-left last:rounded-b-lg first:rounded-t-lg text-sm text-title hover:bg-gray-200 hover:text-gray-900 dark:text-titleDark dark:hover:bg-blue-900`}
...
const Icon = (props: { isSystemTheme: boolean, isDarkTheme: boolean }) => {
  const { isSystemTheme, isDarkTheme } = props;
  const isSystemDarkTheme = isDarkSystemTheme() && isSystemTheme

  if (isDarkTheme || isSystemDarkTheme) {
    return <MoonIcon className={'h-6 dark:fill-title md:dark:fill-black'} />
  }
...
export const DarkModeToggle = () => {
  ...
  return (
    <>
      <button
        type='button'
        className='inline-flex items-center justify-center rounded-xl p-1 bg-white hover:bg-gray-100 focus:ring-1 focus:ring-gray-200 focus:ring-offset-2 dark:fill-white dark:ring-offset-transparent dark:hover:bg-gray-200 dark:focus:ring-titleDark'
        onClick={() => setIsDropdownOpen(!isDropdownOpen)}
      >
        <Icon isDarkTheme={isDarkTheme} isSystemTheme={isSystemTheme} />
      </button>
      {isDropdownOpen && (

...
First, you should be familiar with [Yuma Consensus](/docs/subspace/yuma-consensus) that runs on all subnets except subnet 0. The DAO can provide (and revoke!) you permissions to run without it, but that only makes sense in edge-cases.
...
Learn more about what Subnet Parameters exist [here!](/docs/subnets/subnet-parameter)
